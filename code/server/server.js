...
//GET /api/skus/:id
app.get("/api/skus/:id", stripColonFromParam("id"), param("id").isInt({ min: 1 }), async (req, res) => {
  ...
  try {
    ...
    if (sku.position) sku.position = sku.position.id;
    sku.testDescriptors = sku.testDescriptors.map((t) => t.id);
    return res.status(200).json(sku);
  } catch (err) {
    if (err === EzWhException.InternalError) return res.status(500).end();
    else if (err === EzWhException.NotFound) return res.status(404).end();
  }
});
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      else if (err === EzWhException.PositionFull) return res.status(422).end();
...
//DELETE /api/skus/:id
app.delete("/api/skus/:id", stripColonFromParam("id"), param("id").isInt({ min: 1 }), async (req, res) => {
  const validationErrors = validationResult(req);
  if (!validationErrors.isEmpty()) {
    return res.status(422).end();
  }
...
app.get("/api/skuitems/sku/:id", stripColonFromParam("id"), param("id").isInt({ min: 1 }), async (req, res) => {
  const validationErrors = validationResult(req);
  if (!validationErrors.isEmpty()) {
    return res.status(422).end();
  }
...
app.get("/api/skuitems/:rfid", stripColonFromParam("rfid"), param("rfid").exists(), async (req, res) => {
  const validationErrors = validationResult(req);
  if (!validationErrors.isEmpty()) {
    return res.status(422).end();
  }
...
app.delete("/api/skuitems/:rfid", stripColonFromParam("rfid"), param("rfid").exists(), async (req, res) => {
  const validationErrors = validationResult(req);
  if (!validationErrors.isEmpty()) {
    return res.status(422).end();
  }
